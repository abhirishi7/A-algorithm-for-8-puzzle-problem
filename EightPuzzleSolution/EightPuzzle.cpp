...
private:
	...
	int colSize;
	int blankValue;
	queue<stateNode*> activeNodes; // queue of all nodes that are generated but not yet expanded or visited.
	vector<stateNode*> activeNodesAux;
	vector<stateNode*> visitedNodes; // list of all visited or expanded nodes.
	stateNode* candidateSolution; // a possible goal state node.
...
public:
	...
	bool isQueueEmpty(queue<stateNode*>);
	vector<int*> getValidMoves(stateNode*);
	void generateNextStates(stateNode*);
	void checkCandidateSolution(stateNode*);
...
void SearchTree::printSolutionPath(stateNode* solutionNode) {
	...
	cout << endl << "Total levels traversed : " << levelCount - 1;
...
void SearchTree::selectNextBestStates(vector<stateNode*> nodes) {
	...
	int minCost = 9999;
	for (unsigned int i = 0; i < nodes.size(); i++) { // find the minimum cost among all nodes.
		if (nodes[i]->cost < minCost)
			minCost = nodes[i]->cost;
	}
	for (unsigned int i = 0; i < nodes.size(); i++)
		if (nodes[i]->cost == minCost) { // if node has cost == minimum cost, it is one of the best nodes.
...
void SearchTree::generateNextStates(stateNode* parentNode) {
	...
	for (unsigned int i = 0; i < validMoves.size(); i++) { // for each valid move, generate the state node and fill out all properties.
		...
		newNode->cost = getHeuristicCost(newNode); // assign the heuristic cost to this new state node.
...
void SearchTree::checkCandidateSolution(stateNode* currNode) {
	if (candidateSolution == NULL)
		candidateSolution = currNode;
	else
		candidateSolution =
				(candidateSolution->distance < currNode->distance) ?
						candidateSolution : currNode;
}
...
bool SearchTree::isNodeAlreadyGenerated(stateNode* node) {
	...
	for (unsigned int i = 0; i < activeNodesAux.size(); i++) { // for each node in the activeNodes queue, check if any of those
		...
		if (count == (rowSize * colSize)) { // if count == (rowSize * colSize), means that all the numbers in the state matches with the particular node
			if (activeNodesAux[i]->distance > node->distance) { // from the active nodes queue.
...
void SearchTree::findSolution() {
	...
	while (!isQueueEmpty(activeNodes)) { // loop while queue of active nodes is not empty.
		stateNode* currNode = activeNodes.front(); // pop the first element from queue.
		activeNodes.pop();
 ...
 */
bool SearchTree::isQueueEmpty(queue<stateNode*> queue) {
	return !queue.size();
}
...
int main() {
	...
	clock_t start = clock();
	cout << endl << "Search tree creation started...";
